 ...
 * Execution model: one instruction takes several clock cycles, sequenced by the FSM in the 'controller' module:
    fetch -> decode -> read_src_regs -> execute -> [rw_data_mem] -> write_back -> call_nxt_pc -> fetch ...
 * pc and instr stay constant for the whole of one instruction, so every combinational signal derived from instr (WBMUX, IRMUX, immediates, ...)
   is stable across all the states of that instruction. The write-back and next-PC logic both rely on that property.
**/
...
module risc_v(
    ...
    /**
      JALR target latch.
       Jump_target is combinational: JT = rs1 + immI, read live off the register file. prgm_cntr
       samples it at the call_nxt_pc -> fetch edge, but the JALR link value (rd = pc+4) is written
       one edge earlier, at write_back -> call_nxt_pc. When rd == rs1, that write lands first, so by
       the time call_nxt_pc is reached the async rs1 read already reflects the overwritten value and
       Jump_target is corrupted.
       Fix: latch Jump_target every clock edge. write_back unconditionally precedes call_nxt_pc for
       every instruction in this FSM, so the latched copy always holds the value that was live during
       write_back - i.e. before the register write - by the time call_nxt_pc needs it.
    **/
    reg [31:0] JT_latched;
    always@(posedge clock) JT_latched <= Jump_target;
    ...
    wire [3:0]  dmem_wstrb = MWR ? 4'b0000: store_strb;  // MWR is low only during rw_data_mem of a Store, so it qualifies the strobes.

    /**
      Write-back source mux.
       ALUOP is held for the whole instruction in controller and the data memory read is combinational, so every
       source below is stable from 'execute' onward - i.e. still valid during 'write_back', which is the only 
       state where WERF is asserted.
    **/
    always@(*) begin
        case(WBMUX)
 ...
 *  RV32M ops are 5'b10_<funct3>, so the decoder can pass funct3 straight through:
...
module ALU(
    ...
    always@(*) begin
        ...
        case(op_code)
            ...
            // RV32M operations: the multiply/divide unit is enabled and produces a result
            5'd16: ALU_out = mult_LB;  // MUL: low 32 bits
            5'd17,5'd18, 5'd19: ALU_out = mult_HB;  // MULH / MULHSU / MULHU: high 32 bits
...
module controller(
    ...
    always@(*) begin
        case(present_state)
            ...
            read_src_regs: next_state = execute;
            execute: begin
            ...
            write_back: next_state = call_nxt_pc;
            call_nxt_pc: next_state = fetch;
...
module data_mem(
    ...
    always@(posedge data_mem_clk) begin
        if(data_mem_rst) begin
            ...
            mem[word_addr][7:0] <= (wstrb[0]) ? data_in[7:0] : mem[word_addr][7:0];
            mem[word_addr][15:8] <= (wstrb[1]) ? data_in[15:8] : mem[word_addr][15:8];
            mem[word_addr][23:16] <= (wstrb[2]) ? data_in[23:16] : mem[word_addr][23:16];
            mem[word_addr][31:24] <= (wstrb[3]) ? data_in[31:24] : mem[word_addr][31:24];
        end
    end
endmodule
 ...
 * The data memory is organised as 32-bit words, but RV32I addresses memory by byte. 
   This block sits between the two and handles both directions.
...
module lsu_align(
    ...
    always@(*) begin
        ...
        // extending it to 32 bits after selecting addressed sub-word, with sign or zero extension as appropriate
        case(funct3)
...
module reg_file(
    ...
    always@(posedge reg_file_clk) begin
        if(reg_file_rst) begin
            ...
            mem[0] <= 32'd0; // x0 is always zero

            if(write_enb && (write_addr != 5'd0)) mem[write_addr] <= write_data;
